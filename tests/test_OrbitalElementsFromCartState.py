...
class Test_OrbitalElementsFromCartState(unittest.TestCase):
    ...
    def test_OrbitPeriodFromSma(self):
        ...
        #self.assertAlmostEqual(OrbitalElementsFromCartState.OrbitPeriodFromSma(175521278.778045,398600.5*1000000000), 731822.2514286994) #solution from python method
        self.assertAlmostEqual(OrbitalElementsFromCartState.OrbitPeriodFromSma(175521278.778045,398600.5*1000000000), 731822.2514287010, 8) #solution from excel macro

        #Error handling
        #option 1: 
        #self.assertRaises(ZeroDivisionError, OrbitalElementsFromCartState.OrbitPeriodFromSma, 16, 0)
    ...
    def test_SemimajorAxisFromCartState(self):
        self.assertAlmostEqual(OrbitalElementsFromCartState.SemimajorAxisFromCartState(164335595.593791, 1606.26969960476,398600.5*1000000000),175521278.77804464, 8)
        with self.assertRaises(ZeroDivisionError):
            OrbitalElementsFromCartState.SemimajorAxisFromCartState(0, 1606.26969960476,398600.5*1000000000)


    def test_OrbitEnergyFromCartState(self):
        self.assertAlmostEqual(OrbitalElementsFromCartState.OrbitEnergyFromCartState(164335595.593791, 1606.26969960476,398600.5*1000000000),-1135476.28747637, 7)
        with self.assertRaises(ZeroDivisionError):
            OrbitalElementsFromCartState.OrbitEnergyFromCartState(0, 1606.26969960476,398600.5*1000000000)
